...
from radio.serial_link import (SerialLink, LinkDown, MIN_SEND_GAP_S,
                              DEFAULT_SETTLE_S)

CP210X_VID = 0x10C4     # Heltec V3 onboard USB-serial (Silicon Labs CP2102)

# Pacing and the settle delay live in radio/serial_link.py now.
...
class Link(SerialLink):
    """The bench tools' link: waits out the boot banner, shows non-protocol
    lines, and raises when the stick is gone so a person is told."""

    def __init__(self, port, baud=115200, settle=DEFAULT_SETTLE_S):
        super().__init__(port, baud, settle=settle, report_raw=True,
                         raise_on_down=True,
                         log=lambda m: print(f"{C_ERR}{m}{C_OFF}", flush=True))
